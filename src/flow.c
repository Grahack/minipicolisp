...
any doEval(any x) {
   ...
   if (!isNum(y = EVAL(car(x))) || !(p = Env.bind))
      ...
      struct {  // bindFrame
         ...
         int i, cnt;
         struct {any sym; any val;} bnd[100]; //length(x)];
      } f;
...
static any evMethod(any o, any expr, any x) {
   ...
   struct {  // bindFrame
      struct bindFrame *link;
      int i, cnt;
      struct {any sym; any val;} bnd[100]; //length(y)+3];
   } f;
...
any doBind(any ex) {
   ...
   }
   {
      struct {  // bindFrame
         struct bindFrame *link;
         int i, cnt;
         struct {any sym; any val;} bnd[100]; //length(y)];
      } f;
...
any doLet(any x) {
   ...
   if (!isCell(y = car(x))) {
      ...
      struct {  // bindFrame
         struct bindFrame *link;
         int i, cnt;
         struct {any sym; any val;} bnd[100]; //(length(y)+1)/2];
      } f;
...
any doFor(any x) {
   ...
   struct {  // bindFrame
      struct bindFrame *link;
      int i, cnt;
      struct {any sym; any val;} bnd[100]; //2];
   } f;
...
static struct {  // bindFrame
   struct bindFrame *link;
   int i, cnt;
   struct {any sym; any val;} bnd[100]; //2];  // for 'Up' and 'At'
} Brk;
